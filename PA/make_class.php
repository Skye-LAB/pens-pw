...
if (isset($_POST["make_class"])) {
  if (make_class($_SESSION["user"]["id"], $_POST) > 0) {
    echo "<script>
              alert('Class was made');
              document.location.href = 'my_classes.php'
            </script>";
  } else {
    echo "<script>
              alert('Class was not made successfully');
              document.location.href = 'my_classes.php'
            </script>";
  }
}
  ...
  <main>
    <form action="" method="post">
      ...
      <div class="d-flex align-items-center justify-content-end my-4">
        <button type="submit" name="make_class" class="btn btn-primary ms-4">Create</button>
      </div>
    </form>
  </main>
